...
const Repo = ({repo}) => {
	...
	return (
		...
		text-gray-400'
			>
				Release on {formatedDate}
			</time>
			<p className='mb-4 text-base font-normal text-gray-500'>{repo.description?repo.description.slice(0,200):"No description provided"}</p>
			{PROGRAMMING_LANGUAGES[repo.language]?(
				<img src={PROGRAMMING_LANGUAGES[repo.language]} alt='image for lang' className="h-8 cursor-pointer"/>
			):null}
		</li>
	);
};
